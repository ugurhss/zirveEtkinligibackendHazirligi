    ...
    <div class="row grid-responsive">
        <div class="column">
            ...
            <div class="header-container" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                <form action="{{ route('admin.messages.index') }}" method="GET" style="display: flex; max-width: 500px; width: 100%;">
                    <input
                        type="text"
                        name="search"
                                ...
                                @forelse($messages as $message)
                                    <tr style="border-bottom: 1px solid #ccc;">
                                        ...
                                        <td style="padding: 10px;">
                                            <a href="{{ route('admin.messages.show', $message->id) }}" style="color: #007bff; margin-right: 10px;">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                            <form action="{{ route('admin.messages.destroy', $message->id) }}" method="POST" style="display: inline;">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" onclick="return confirm('Silmek istediğinize emin misiniz?')" style="color: red; border: none; background: none;">
